...
<div class="panel panel-white">
    <div class="panel-heading">
        <h6 class="panel-title">Waktu Respon: <small>{{ $response->created_at->format('jS F, Y g:i a') }} ({{ $response->created_at->diffForHumans() }})</small></h6>
        <div class="heading-elements">
            ...
            @if (!empty($response_data))
                <a href="javascript:void(0)" id="delete-button" data-href="{{ route('forms.responses.destroy.single', [$form->code, $response->id]) }}" data-item="respon" class="btn btn-sm btn-danger position-right">Hapus Respon</a>
            @endif
        </div>
    </div>
                    ...
                    @php
						...
						if ($data['answer']) {
                            if ($data['template'] === 'linear-scale') {
                                ...
                                $range = "(Diantara {$options['min']['value']}{$min_label} dan {$options['max']['value']}{$max_label})";
                                $value = "Rating nya {$data['answer']} {$range}";
                            } else {
                                $value = $data['answer'];
                            }
						} else {
