...
$module = ($view_type === 'preview') ? 'Form Saya' : config('app.name');
        ...
        <div class="panel panel-flat">
            ...
            @if ($form->status === App\Form::STATUS_CLOSED)
                <div class="panel-body">
                    {{ optional($form->availability)->closed_form_message ?? 'Maaf, form ini sudah ditutup.' }}
                </div>
            @else
                <div class="panel-body">
                ...
                <div class="panel-body">
                    <form id="user-form" action="{{ ($view_type === 'form') ? route('forms.responses.store', $form->code) : "#" }}" method="{{ ($view_type === 'form') ? "post" : "get" }}" autocomplete="off">
                        ...
                        <div class="text-left mt-20">
                            <button id="submit" type="{{ ($view_type === 'form') ? 'submit' : 'button' }}" class="btn btn-primary" data-loading-text="Mohon tunggu.." data-complete-text="Submit Form">Simpan</button>
                        </div>
                    </form>
                </div>
            @endif
